...
(function(window){var config=get_config(),$cache=new Cache();dispatcher();function dispatcher(){$cache.setToken(config.options.config['js-version']);config.options.cache=$cache;config.publicJs=get_public_js(config.publicJs,config.options.config['js-version']);config.publicCss=get_public_css(config.publicCss,config.options.config['js-version']);config.seaConfig=get_sea_config(config.seaConfig,config.options.config['js-version']);seajs.config(config.seaConfig);seajs.use(config.publicCss);seajs.use(config.publicJs,function(){var plugIns=arguments;init(function(){$(function(){call_actions(plugIns)})})})}function call_actions(plugIns){for(var i in lxhActions){if(typeof lxhActions[i]=='function'){lxhActions[i].apply(this,plugIns)}}}function init(call){window.$lxh=new Lxh(config.options);var lang=$lxh.config().get('language');var serverOptions=$lxh.createStore({});if(typeof load_data=='function'){serverOptions.set(load_data())}$lxh.language().type(lang);$lxh.language().fill(serverOptions.get('language')||null,true);$lxh.tpl().fill(serverOptions.get('tpl')||null,true);$('[data-pattern]').each(function(){var $tableScrollWrapper=$(this);if(typeof $tableScrollWrapper.responsiveTable!='undefined'){$tableScrollWrapper.responsiveTable($tableScrollWrapper.data())}});call()}function get_lang_cache_key(lang){return'language_'+lang}function check_cache_language(lang,scopes,useCache){var cacheKey=get_lang_cache_key(lang),package=$cache.get(cacheKey),t=[],i;if(typeof add_lang_scopes=='function'){var addScopes=add_lang_scopes();for(i in addScopes){scopes.push(addScopes[i])}}if(!package||!useCache)return scopes||[];for(i in scopes){if(!package[scopes[i]])t.push(scopes[i])}return t||[]}function get_public_css(publicCss,v){if(typeof cssLibArr!='undefined'){cssLibArr=array_unique(cssLibArr);for(var i in cssLibArr){publicCss.push(cssLibArr[i])}}for(i in publicCss){publicCss[i]=publicCss[i]+'?v='+v}return publicCss}function get_public_js(publicJs,version){if(typeof jsLibArr!='undefined'){jsLibArr=array_unique(jsLibArr);for(var i in jsLibArr){publicJs.push(jsLibArr[i]+'.js?v='+version)}}var scopes=check_cache_language(config.options.config.language,config.langScopes,config.options.config['use-cache']);var loads={};if(scopes.length>0){loads.language=scopes.join(',')}var jsApi=get_load_data_js_api(loads);if(jsApi){publicJs.unshift(jsApi)}return publicJs;function get_load_data_js_api(data){var api=config.options.dataApi;var p='';for(var i in data){p+='&n[]='+i+':'+data[i]}if(p){return api+'?'+p}return''}}function get_sea_config(config,version){for(var i in config.alias){config.alias[i]=config.alias[i]+'.js?v='+version}return config}function Cache(){this.storage=window.localStorage||{};this.token=null;this.prefix={general:"$lxh_",timeout:"@lxh_"};this.setToken=function(token){this.token=token};this.saveToken=function(token){this.set('$$token',token||this.token)};this.set=function(key,val){if(val instanceof Object){val=JSON.stringify(val)}this.storage.setItem(this.prefix.general+key,val)};this.get=function(key,def){if(!this.checkTokenValid(key)){return def||null}if(this.clearTimeout(key))return null;var val=this.storage.getItem(this.prefix.general+key);if(val){if(val.indexOf("{")===0||val.indexOf("[")===0){return JSON.parse(val)}return val}return(def||null)};this.checkTokenValid=function(key){if(key=='$$token'){return true}if(this.token!=this.get('$$token')){this.clearAll();this.saveToken();return false}return true};this.clearPastDueKey=function(){for(var key in this.storage){if(key.indexOf(this.prefix.timeout)==-1){continue}this.clearTimeout(key.replace(this.prefix.timeout,""))}};this.clearTimeout=function(key){var d,timeoutKey=this.prefix.timeout+key,timeout=this.storage.getItem(timeoutKey);if(timeout){d=new Date().getTime();if(timeout<d){delete this.storage[this.prefix.general+key];delete this.storage[timeoutKey];return true}}return false};this.expire=function(key,timeout){var d=new Date().getTime()+(parseInt(timeout));this.storage.setItem(this.prefix.timeout+key,d)};this.expireAt=function(key,timeout){this.storage.setItem(this.prefix.timeout+key,timeout)};this.clearAll=function(){for(var i in this.storage){delete this.storage[i]}};this.clearPastDueKey()}})(window);