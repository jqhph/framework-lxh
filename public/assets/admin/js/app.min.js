...
(function (w) {
    var cache = LXHSTORE.cache,
        ...
        useCache = LXHSTORE.saveScript,
        configs = LXHSTORE.seaConfig;

    function Load(src, completed) {
        var queue = [];
...
(function (window) {
    ...
    function dispatch() {
        ...
        // 加载css
        new LxhLoad(get_used_css(config.publicCss, cssversion));

        new LxhLoad(get_used_js(config.publicJs, jsversion), function () {
            setTimeout(init, 10);
        });
    }
