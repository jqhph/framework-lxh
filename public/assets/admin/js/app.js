...
(function (window) {
    var config = __ini__(), $cache = new Cache();

    dispatcher();

    function dispatcher() {
        var jsversion = config.options.settings['js-version'], cssversion = config.options.settings['css-version'];
        // 设置缓存token
        ...
        $(document).on('app.reload', function () {
            // 加载css
    ...
    function call_actions(plugIns) {
        ...
        lxhActions = [];
        $(document).trigger('app.completed');
    }
