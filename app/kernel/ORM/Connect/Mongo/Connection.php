...
//         }
		$this->config['hostname'] = Arr::getValue($config, 'host');
		$this->config['database'] = Arr::getValue($config, 'db');
		$this->config['username'] = Arr::getValue($config, 'user');
		$this->config['password'] = Arr::getValue($config, 'pwd');
		$this->config['hostport'] = Arr::getValue($config, 'port');
		$this->config['charset']  = Arr::getValue($config, 'charset', 'utf8');
    }
    ...
    public function connect(array $config = [], $linkNum = 0)
    {
        if (!isset($this->links[$linkNum])) {
            ...
            if ($config['debug']) {
                // 记录数据库连接信息
                logger()->info('[DB] CONNECT:[ UseTime:' . number_format(microtime(true) - $startTime, 6) . 's ] ' . $config['dsn'], 'sql');
            }
        }
