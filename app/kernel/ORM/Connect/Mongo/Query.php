...
// +----------------------------------------------------------------------
namespace Lxh\ORM\Connect\Mongo;
...
// use think\Db;
use \Exception;
use Lxh\ORM\Connect\Mongo\Builder;
use Lxh\ORM\Connect\Mongo\Connection;
...
class Query
{
    ...
    {
        $this->connection = $connection ?: app('mongo.connect');
...
//             $result = Cache::get($key);
        ...
        if (!$result) {
            ...
            $cursor = $this->field($field)->fetchCursor(true)->findOne();
...
//             $result = Cache::get($guid);
        ...
        if (!$result) {
            ...
            $cursor = $this->field($field)->fetchCursor(true)->read();
            $cursor->setTypeMap(['root' => 'array']);
            $resultSet = $cursor->toArray();
            if ($resultSet) {
...
//         }
    ...
    {
        $config   = array_merge(C('paginate'), $config);
        ...
        if (!$simple) {
            ...
            $total   = $this->count();
            $results = $this->options($options)->page($page, $listRows)->read();
        } else {
            $results = $this->limit(($page - 1) * $listRows, $listRows + 1)->read();
            $total   = null;
        }
...
//         }
        if (!$resultSet) {
            // 生成MongoQuery对象
            $query = $this->builder->read($options);
            // 执行查询操作
...
//             }
    ...
    {
        if (!empty($this->model)) {
            throw new Exception('model data Not Found:' . $this->model, $this->model, $options);
        } else {
            throw new Exception('table data not Found:' . $options['table'], $options['table'], $options);
        }
    }
    ...
    {
        return $this->failException(true)->read($data);
    }
    ...
    {
        return $this->failException(true)->findOne($data);
    }
    ...
    public function chunk($count, $callback, $column = null)
    {
        ...
        $options   = $this->getOptions();
        $resultSet = $this->limit($count)->order($column, 'asc')->read();
    ...
    public function getTableInfo($tableName = '', $fetch = '')
    {
        ...
        if (!isset(self::$info[$guid])) {
            $result = $this->table($tableName)->findOne();
