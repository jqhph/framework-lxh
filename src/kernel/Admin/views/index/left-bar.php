        ...
        <div id="sidebar-menu">
            <ul>
                ...
                <?php foreach ($menu->get() as & $m) { ?>
                    <li class="has_sub">
                        <a id="menu-<?php echo $m['id']?>" href="javascript:<?php echo empty($m['subs']) && !empty($m['controller']) ? "LXHSTORE.TAB.switch('menu-{$m['id']}', '{$m['url']}', '{$m['name']}')" : '';?>"
                                ...
                                <?php foreach ($m['subs'] as & $sub) { ?>
                                    <li class="<?php if ($menu->isActive($sub['controller'], $sub['action'])) echo 'active';?>">
                                        <a id="menu-<?php echo $sub['id']?>" class="<?php if ($menu->isActive($sub['controller'], $sub['action'])) echo 'active';?>"
                                           href="javascript:LXHSTORE.TAB.switch('menu-<?php echo $sub['id'];?>', '<?php echo $sub['url'];?>', '<?php echo $sub['name']?>')">
