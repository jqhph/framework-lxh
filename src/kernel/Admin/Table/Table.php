 ...
 * @method Table date($field);
 * @method Table icon($field);
 * @method Table select($field);
 * @method Table text($field);
 */
...
EOF;
    ...
    public function __call($method, $parameters)
    {
        if (isset(static::$fields[$method])) {
            return $this->field(get_value($parameters, 0))->view(static::$fields[$method]);
        }

        if (isset($this->builderViewTypes[$method])) {
            $this->field(get_value($parameters, 0));
